...
class DbManager:
    ...
    def addUniversity(self, university_name):
        query = DbQueriesController().getInsertQuery(
            "universities", "university_name", university_name)
    ...
    def getUniversityIdByName(self, name):
        query = DbQueriesController().getSelectWithParamQuery(
            "university_id", "universities", "university_name", name)
        return SqlLiteDbController().fetchQuery(query)
    ...
    def getGroupId(self, groupInfo):
        query = DbQueriesController().getGroupIdQuery(
            groupInfo.get('group_name'), groupInfo.get('university_id'))
        return SqlLiteDbController().fetchQuery(query)[0][0]

    def getGroupJsonById(self, groupId):
        query = DbQueriesController().getSelectWithParamQuery(
            'schedule_text', 'groups', 'group_id', groupId)
        return SqlLiteDbController().fetchQuery(query)

    def getGroupsByUniversityId(self, universityId):
        query = DbQueriesController().getSelectWithParamQuery(
            "group_id, group_name", "groups", "university_id", universityId)
        return SqlLiteDbController().fetchQuery(query)

    def addTgUser(self, userInfo):
        query = DbQueriesController().checkIfExist(
            "telegramUsers", "user_id", userInfo.get("user_id"))
    ...
    def updateTgUser(self, user_id, paramName, paramVal):
        query = DbQueriesController().getUpdateQuery(
            "telegramUsers", paramName, paramVal, "user_id", user_id)
        SqlLiteDbController().submitQuery(query)

    def getTgUserInfo(self, user_id):
        query = DbQueriesController().getSelectWithParamQuery(
            "*", "telegramUsers", "user_id", user_id)
        return SqlLiteDbController().fetchQuery(query)
